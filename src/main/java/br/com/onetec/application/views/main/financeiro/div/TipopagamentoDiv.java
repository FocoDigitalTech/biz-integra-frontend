...
import br.com.onetec.application.views.main.financeiro.modal.TipoPagamentoDetalheModal;
...
@Component
@UIScope
public class TipopagamentoDiv extends Div {

    private Grid<SetTipoPagamento> grid;

    private TipopagamentoDiv.Filter filter;
    ...
    private TipoPagamentoDetalheModal tipoPagamentoDetalheModal;
    ...
    public void initServices(UtilitySystemConfigService service1,
                             ...
                             TipoPagamentoService tipoEventoFinanceiroService1,
                             TipoPagamentoCadastroModal tipoPagamentoCadastroModal1,
                             TipoPagamentoDetalheModal tipoPagamentoDetalheModal1) {
        this.tipoPagamentoService = tipoEventoFinanceiroService1;
    ...
    private Div telaDiv() {
        ...
        filter = new TipopagamentoDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetTipoPagamento.class, false);
        grid.addColumn(SetTipoPagamento::getId_tipopagamento)
                .setHeader("Id")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetTipoPagamento::getNome_tipopagamento)
                .setHeader("Nome")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetTipoPagamento::getDescricao_tipopagamento)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetTipoPagamento::getData_inclusao)
        ...
        grid.setItems(query -> tipoPagamentoService.list(
        ...
        grid.addItemClickListener(event -> {
            ...
            UI.getCurrent().access(() -> {
                tipoPagamentoDetalheModal.setTipoPagamento(event.getItem());
...
//            });
    ...
    public class Filter extends Div implements Specification<SetTipoPagamento> {
        ...
        @Override
        public Predicate toPredicate(Root<SetTipoPagamento> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_tipopagamento"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "nome_tipopagamento";
                String ignore = "- ()";
