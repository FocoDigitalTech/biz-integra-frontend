...
@Component
@UIScope
public class ExecucaoServicoDiv extends Div {

    private Grid<SetExecucaoServico> grid;
    ...
    private ExecucaoServicoDiv.Filter filter;
    ...
    private Div telaDiv() {
        ...
        filter = new ExecucaoServicoDiv.Filter(() -> refreshGrid());
    ...
    private void deleta(SetExecucaoServico item) {
        try {
            execucaoServicoService.delete(item);
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetExecucaoServico.class, false);
        grid.addColumn(SetExecucaoServico::getId_execucaoservico)
                .setHeader("ID")
                //.setSortable(true)
                .setAutoWidth(true);

        grid.addColumn(SetExecucaoServico::getNome_execucaoservico)
                .setHeader("Nome Serviço OS")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetExecucaoServico::getDescricao_execucaoservico)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
        ...
        grid.setItems(query -> execucaoServicoService.list(
    ...
    public class Filter extends Div implements Specification<SetExecucaoServico> {
        ...
        @Override
        public Predicate toPredicate(Root<SetExecucaoServico> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_execucaoservico"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "nome_execucaoservico";
                String ignore = "- ()";
