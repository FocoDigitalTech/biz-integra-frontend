...
import br.com.onetec.application.views.main.financeiro.modal.GrupoFinanceiroDetalhesModal;
...
@Component
@UIScope
public class GrupoFinanceiroDiv extends Div {

    private Grid<SetGrupoFinanceiro> grid;

    private GrupoFinanceiroDiv.Filter filter;

    private UtilitySystemConfigService service;

    private GrupoFinanceiroService grupoFinanceiroService;

    private GrupoFinanceiroCadastroModal grupoFinanceiroCadastroModal;

    private GrupoFinanceiroDetalhesModal grupoFinanceiroDetalhesModal;
    ...
    public void initServices(UtilitySystemConfigService service1,
                             ...
                             GrupoFinanceiroCadastroModal grupoFinanceiroCadastroModal1,
                             GrupoFinanceiroDetalhesModal grupoFinanceiroDetalhesModal1) {
        this.grupoFinanceiroService = grupoFinanceiroService1;
    ...
    private Div telaDiv() {
        ...
        filter = new GrupoFinanceiroDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetGrupoFinanceiro.class, false);
        grid.addColumn(SetGrupoFinanceiro::getId_grupoeventofinanceiro)
                .setHeader("Id")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetGrupoFinanceiro::getNome_grupoeventofinanceiro)
                .setHeader("Nome")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetGrupoFinanceiro::getDescricao_grupoeventofinanceiro)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
        ...
        grid.addItemClickListener(event -> {
            ...
            UI.getCurrent().access(() -> {
                grupoFinanceiroDetalhesModal.setGrupoFinanceiro(event.getItem());
                grupoFinanceiroDetalhesModal.open();
            });
...
//            });
    ...
    public class Filter extends Div implements Specification<SetGrupoFinanceiro> {
        ...
        @Override
        public Predicate toPredicate(Root<SetGrupoFinanceiro> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_grupoeventofinanceiro"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "nome_grupoeventofinanceiro";
                String ignore = "- ()";
