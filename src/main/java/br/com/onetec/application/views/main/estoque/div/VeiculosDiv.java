...
@Component
@UIScope
public class VeiculosDiv extends Div {

    List<SetProduto> list = new ArrayList<>();
    private Grid<SetVeiculo> grid;

    private VeiculosDiv.Filter filter;
    ...
    private Div telaDiv() {
        ...
        filter = new VeiculosDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetVeiculo.class, false);
        grid.addColumn(SetVeiculo::getId_veiculo)
                .setHeader("ID")
                //.setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetVeiculo::getNome_veiculo)
                .setHeader("Nome")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetVeiculo::getCombustivel_veiculo)
                .setHeader("Combustivel")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetVeiculo::getMarca_veiculo)
                .setHeader("Marca")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetVeiculo::getModelo_veiculo)
                .setHeader("Modelo")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetVeiculo::getPlaca_veiculo)
                .setHeader("Placa")
                .setSortable(true)
                .setAutoWidth(true);
        ...
        grid.addItemDoubleClickListener(event -> {
            SetVeiculo produto = event.getItem();
...
//        dialog.open();
    ...
    public class Filter extends Div implements Specification<SetVeiculo> {
        ...
        @Override
        public Predicate toPredicate(Root<SetVeiculo> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_veiculo"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "nome_veiculo";
                String ignore = "- ()";
