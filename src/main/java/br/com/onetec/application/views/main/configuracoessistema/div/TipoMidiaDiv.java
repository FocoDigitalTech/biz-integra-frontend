...
public class TipoMidiaDiv extends Div{
    ...
    private TextField descricao_tipomidia;
...
//        });
    ...
    private Div telaDiv() {
        ...
        filter = new TipoMidiaDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetTipoMidia.class, false);
        grid.addColumn(SetTipoMidia::getId_tipomidia)
                .setHeader("ID")
                //.setSortable(true)
                .setAutoWidth(true);

        grid.addColumn(SetTipoMidia::getDescricao_tipomidia)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
    ...
    private void deleta(SetTipoMidia item) {
        try {
    ...
    public class Filter extends Div implements Specification<SetTipoMidia> {
        ...
        @Override
        public Predicate toPredicate(Root<SetTipoMidia> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_tipomidia"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "descricao_tipomidia";
                String ignore = "- ()";
