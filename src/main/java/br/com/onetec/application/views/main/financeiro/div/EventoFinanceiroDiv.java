...
@Component
@UIScope
public class EventoFinanceiroDiv extends Div {
    private Grid<SetEventoFinanceiro> grid;

    private EventoFinanceiroDiv.Filter filter;
    ...
    private Div telaDiv() {
        ...
        filter = new EventoFinanceiroDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetEventoFinanceiro.class, false);
        grid.addColumn(SetEventoFinanceiro::getId_eventofinanceiro)
                .setHeader("Id")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetEventoFinanceiro::getNome_eventofinanceiro)
                .setHeader("Nome")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetEventoFinanceiro::getObservacoes_eventofinanceiro)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
    ...
    public class Filter extends Div implements Specification<SetEventoFinanceiro> {
        ...
        @Override
        public Predicate toPredicate(Root<SetEventoFinanceiro> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_eventofinanceiro"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "nome_eventofinanceiro";
                String ignore = "- ()";
