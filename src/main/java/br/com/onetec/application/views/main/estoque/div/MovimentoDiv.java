...
@Component
@UIScope
public class MovimentoDiv extends Div {

    List<SetProduto> list = new ArrayList<>();
    private Grid<SetEstoque> grid;

    private MovimentoDiv.Filter filter;
    ...
    private Div telaDiv() {
        ...
        filter = new MovimentoDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetEstoque.class, false);
        grid.addColumn(SetEstoque::getId_estoque)
                .setHeader("Id")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(estoque -> {
            SetProduto produto = produtoService.findById(estoque.getId_produto());
            return produto == null ? "N/A" : produto.getNome_produto();
        })
                .setHeader("Nome Produto")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetEstoque::getQuantidade_consumida)
                .setHeader("Quantidade Consumida")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetEstoque::getQuantidade_devolvida)
        ...
        grid.addItemDoubleClickListener(event -> {
            SetEstoque estoque = event.getItem();
            ...
            abrirDetalhesDoEstoque(estoque);
        });
    ...
    public class Filter extends Div implements Specification<SetEstoque> {
        ...
        @Override
        public Predicate toPredicate(Root<SetEstoque> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_estoque"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "numero_lote";
                String ignore = "- ()";
