...
@Component
@UIScope
public class CondicaoPagamentoDiv extends Div {

    List<SetProduto> list = new ArrayList<>();
    private Grid<SetCondicaoPagamento> grid;

    private CondicaoPagamentoDiv.Filter filter;
    ...
    private Div telaDiv() {
        ...
        filter = new CondicaoPagamentoDiv.Filter(() -> refreshGrid());
    ...
    private com.vaadin.flow.component.Component createGrid() {
        ...
        grid = new Grid<>(SetCondicaoPagamento.class, false);
        grid.addColumn(SetCondicaoPagamento::getId_condicaopagamento)
                .setHeader("Id")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetCondicaoPagamento::getDescricao_condicaopagamento)
                .setHeader("Descrição")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetCondicaoPagamento::getQuantidade_parcelas)
                .setHeader("Quantidade Parcelas")
                .setSortable(true)
                .setAutoWidth(true);
        grid.addColumn(SetCondicaoPagamento::getData_inclusao)
    ...
    public class Filter extends Div implements Specification<SetCondicaoPagamento> {
        ...
        @Override
        public Predicate toPredicate(Root<SetCondicaoPagamento> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
            List<Predicate> predicates = new ArrayList<>();


            if (!id.isEmpty()) {
                Integer lowerCaseFilter = Integer.valueOf(id.getValue().toLowerCase());
                Predicate idMatch = criteriaBuilder.equal(root.get("id_condicaopagamento"), lowerCaseFilter);
                predicates.add(criteriaBuilder.or(idMatch));
            }

            if (!nome.isEmpty()) {
                String databaseColumn = "descricao_condicaopagamento";
                String ignore = "- ()";
