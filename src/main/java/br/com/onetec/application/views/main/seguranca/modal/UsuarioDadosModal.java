...
@Component
@UIScope
public class UsuarioDadosModal extends Dialog {
    ...
    private Div createFormCadastro() {
        ...
        entradaPassword.setLabel("Confirme sua Nova Senha");
    ...
    public void setUsuario(SetUsuarios item) {
        UI.getCurrent().access(() -> {
            this.setUsuarios = item;
            id_funcionario.clear();
            id_grupousuario.clear();
            emailField.clear();
            nome_usuario.clear();

            List<SetGrupoUsuario> grupo = grupoUsuarioService.listAll();
            List<SetFuncionario> funcionarios = funcionarioService.listAll();

            id_funcionario.setItems(funcionarios);
            id_funcionario.setValue(funcionarios.stream()
                    .filter(midia -> midia.getId_funcionario().equals(item.getId_funcionario()))
                    .findFirst().orElse(null));

            id_grupousuario.setItems(grupo);
            id_grupousuario.setValue(grupo.stream()
                    .filter(midia -> midia.getId_grupousuario().equals(item.getId_grupousuario()))
                    .findFirst().orElse(null));

            emailField.setValue(item.getEmail_usuario());
            nome_usuario.setValue(item.getNome_usuario());
